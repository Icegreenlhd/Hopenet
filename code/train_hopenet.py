...
def parse_args():
    ...
    parser.add_argument('--data_dir', dest='data_dir', help='Directory path for data.',
                        default='/data/deep-head-pose/300W_LP', type=str)
    parser.add_argument('--filename_list', dest='filename_list',
                        help='Path to text file containing relative paths for every example.',
                        default='/data/deep-head-pose/300W_LP/image.txt', type=str)
...
def gen_detail_name(args):
    outstr = 'model_' + "hopenet" + \
             '_lr_' + str(args.lr) + \
             '_alpha_' + str(args.alpha) + \
             '_epoch-num_' + str(args.num_epochs) + \
             '_gpu' + str(args.gpu_id) + \
             '_batch-size_' + str(args.batch_size) + \
             '_' + str(args.output_string)  # + \ '_' + str(datetime.datetime.now().strftime('%y%m%d_%H%M'))
    return outstr
...
if __name__ == '__main__':
    ...
    if not os.path.exists('output/snapshots/' + gen_detail_name(args)):
        os.makedirs('output/snapshots/' + gen_detail_name(args))
    ...
    for epoch in range(num_epochs):
        for i, (images, labels, cont_labels, name) in enumerate(train_loader):
            ...
            # Cross entropy loss
            # print("yaw: ", yaw, label_yaw)
            # print("pitch: ", pitch, label_pitch)
            # print("roll: ", roll, label_roll)
            ...
            if (i + 1) % 100 == 0:
                print('Epoch [%d/%d], Iter [%d/%d] Losses: Yaw %.4f, Pitch %.4f, Roll %.4f, time: %.4f min'
                      % (epoch + 1, num_epochs, i + 1, len(pose_dataset) // batch_size, loss_yaw.item(),
                         loss_pitch.item(), loss_roll.item(), (time.time() - begin)/60))

        # Save models at numbered epochs.
        if epoch % 1 == 0 and epoch < num_epochs:
            print('Taking snapshot...')
            torch.save(model.state_dict(),
                       'output/snapshots/' + gen_detail_name(args) + '/epoch_' + str(epoch+1) + '.pkl')
